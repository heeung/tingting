...
import com.alsif.tingting.data.model.ErrorDto
...
class AuthInterceptor : Interceptor {
    ...
    private fun getAccessToken(): String? {
        return TingtingApp.authDataSource.getAccessToken()
    }

    // RefreshToken을 가져옵니다.
    private fun getRefreshToken(): String {
        return requireNotNull(TingtingApp.authDataSource.getRefreshToken()) { NO_REFRESH_TOKEN }
    }

    // Token을 저장합니다.
    private fun storeToken(accessToken: String, refreshToken: String) {
        TingtingApp.authDataSource.setAccessToken(accessToken)
        TingtingApp.authDataSource.setRefreshToken(refreshToken)
    }
